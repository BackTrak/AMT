			...
			case 9:
				{
					...
					else
					{
						...
						if (l2type == "ImportImage")
						{
							...
							cf.Read(&(img->bw),sizeof(long)); // scanwidth = width * 2 (rounded to upper long)
							...
							BYTE *lpBits = new BYTE[nbits];
							cf.Read(lpBits,nbits);
							img->bitmap = lpBits;
							lastObject = NewMDLObject();
							stackedObjects[++sopos] = lastObject;
...
bool CMDLFile::SaveToBMP(CString fnbmp)
{
	...
	UCHAR *lp24Bits = new UCHAR[img->w*img->h*3];
	...
	for (int y=0;y<img->h;y++)
		...
		{ 
			...
			int p16 = y*img->w+x;
			int p24 = ((img->h-1-y)*img->w+x)*3; // inverted lines (start from bottom)
    ...
    pbmi->bmiHeader.biWidth = img->w; 
    pbmi->bmiHeader.biHeight = img->h; 
...
bool CMDLFile::SaveToFile(CString sPath)
{
	...
	for (int t=0;t<hdr.l3;t++)
	{
		...
		if (!bIsNotImage)
		{
			...
			// save the image 
			MDLImage *img = RootObject->image;
			MDLfile.Write(&(img->w),sizeof(long)); // width 
			MDLfile.Write(&(img->h),sizeof(long)); // height
